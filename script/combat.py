...
def combat(data, batch, estimators=None, model=None, numerical_covariates=None):
    """
    ...
    Args:
        ...
        If not provided, then will attempt to coarsely correct just from the information provided in ``batch``, default=None
        ----------------------------------
        numerical_covariates : (optional) list-like, 
        List of covariates in the model which are numerical, rather than categorical, default=None
    ...
    Returns
        ...
        ----------------------------------
        2. list of 2 np.ndarray, 
        the trained batch effect estimators from the current data.[additive_effect, multiplicative_effect]
    """
    ...
    if estimators.__class__.__name__=='list' and estimators[0].__class__.__name__=='ndarray' and estimators[1].__class__.__name__=='ndarray':
        sys.stderr.write("Imported estimators found\n")
        gamma_star = estimators[0]
        delta_star = estimators[1]
    else:
        sys.stderr.write("No valid imported estimators found\n")
        sys.stderr.write("Calculating estimators\n")
